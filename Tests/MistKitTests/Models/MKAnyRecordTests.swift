...
final class MKAnyRecordTests: XCTestCase {
    ...
    private let expectedStringListField = UUID().uuidString
    private let expectedInt64ListField = UUID().uuidString
    private let expectedDateListField = UUID().uuidString
    ...
    private let expectedStringListValue = ["Hello World"]
    private let expectedInt64ListValue = [Int64(1)]
    private let expectedDateListValue = [Date()]
    ...
    override public func setUp() {
        expectedFields = [
            ...
            expectedStringListField: .stringList(expectedStringListValue),
            expectedDateListField: .dateList(expectedDateListValue),
            expectedInt64ListField: .int64List(expectedInt64ListValue)
    ...
    public func testValues() {
        ...
        XCTAssertEqual(
            try record.stringList(fromKey: expectedStringListField), expectedStringListValue
        )
        XCTAssertEqual(
            try record.int64List(fromKey: expectedInt64ListField), expectedInt64ListValue
        )
        XCTAssertEqual(
            try record.dateList(fromKey: expectedDateListField),
            expectedDateListValue
        )
    }
...
//        valueTest(MKAnyRecord.dateList, UUID().uuidString)
//        valueTest(MKAnyRecord.int64List, UUID().uuidString)
//        valueTest(MKAnyRecord.stringList, UUID().uuidString)
    }
}
